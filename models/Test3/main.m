function output_vars = main
  ...
  function [dphidt, extra_output] = f(t, phi) 
    ...
    N_E_92 = [2];
    S_E_92 = [1];
    an(N_E_92, S_E_92) = E_92(anc, and_x, N_E_92, S_E_92);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    N_E_95 = [2];
    S_E_95 = [1];
    Axy(N_E_95) = E_95(d_x, oneHalf, pi, N_E_95, S_E_95);

    N_E_96 = [2];
    S_E_96 = [1];
    h_L(N_E_96) = E_96(Axy, V, N_E_96, S_E_96);

    extra_output = [V(N_E_36).value, n(N_E_93, S_E_93).value
    ];
...
function sol = E_92(anc, and_x, N, S)
  sol = anc(N, S) + and_x(N, S);
endfunction

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% sys1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function sol = sys1(m, rho, V)
  ...
  function F = sys(x)
    ...
    F(1:1) = res_E_36(m, rho, V, N_E_36, S_E_36);
  endfunction
endfunction

% Residue equations
function sol = res_E_36(m, rho, V, N, S)
  sol = einsum(1 ./ V(N), m(N)) - rho(N);
  sol = sol.value;
endfunction
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function sol = E_95(d_x, oneHalf, pi, N, S)
  sol = einsum(einsum(pi, (einsum(oneHalf, d_x(N)))), (einsum(oneHalf, d_x(N))));
endfunction

function sol = E_96(Axy, V, N, S)
